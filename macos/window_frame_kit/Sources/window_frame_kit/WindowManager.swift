...
public class WindowManager: NSObject, NSWindowDelegate {
    ...
    // // FRAME: custom frame save/restore state (04-03). Unlike
    // setAsFrameless, which is one-way, the toggle is reversible because the
    // pre-toggle decoration snapshot is taken exactly once (idempotent) and
    // restored verbatim on disable.
    private var _savedStyleMask: NSWindow.StyleMask?
    private var _savedTitleVisibility: NSWindow.TitleVisibility?
    private var _savedTitlebarAppearsTransparent: Bool?
    ...
    /// // FRAME: custom frame takeover (04-03). Enables the
    /// fullSizeContentView + transparent-titlebar route while KEEPING the
    /// .titled styleMask bit, so the window stays key/main-capable (a hard
    /// borderless removal breaks focus acquisition - XPLAT-04). All pushed
    /// decoration values are snapshotted and restored on disable.
    public func setCustomFrame(_ args: [String: Any]) {
        let isCustomFrame: Bool = args["isCustomFrame"] as? Bool ?? false
        if isCustomFrame {
            guard _savedStyleMask == nil else { return }  // idempotent re-arm
            _savedStyleMask = mainWindow.styleMask
            _savedTitleVisibility = mainWindow.titleVisibility
            _savedTitlebarAppearsTransparent = mainWindow.titlebarAppearsTransparent
            mainWindow.styleMask.insert(.fullSizeContentView)
            mainWindow.titlebarAppearsTransparent = true
            mainWindow.titleVisibility = .hidden
        } else {
            guard let mask = _savedStyleMask else { return }  // never armed
            mainWindow.styleMask = mask
            if let visibility = _savedTitleVisibility {
                mainWindow.titleVisibility = visibility
            }
            if let transparent = _savedTitlebarAppearsTransparent {
                mainWindow.titlebarAppearsTransparent = transparent
            }
            _savedStyleMask = nil
            _savedTitleVisibility = nil
            _savedTitlebarAppearsTransparent = nil
        }
    }
