...
# plugin.cpp 156 = 原 22 结构偏离 + // FRAME: 嫁接块（03-03 接线 + 2026-09-05
...
if [ "$n1" -ne 156 ]; then
  echo "plugin.cpp residue $n1 != 156"
  fail=1
fi
if [ "$n2" -ne 2 ]; then
...
if [ "$fail" -eq 0 ]; then
  echo "VERBATIM PROOF OK (13 dart zero-diff + 2 customFrame pinned diffs + 1 pinned example deviation, cpp residue 156+2)"
fi
exit "$fail"
